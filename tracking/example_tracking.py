...
def main():
    ...
    tracker = BYTETracker(track_thresh=0.25, track_buffer=100, match_thresh=0.8, frame_rate=30)
    ...
    while zed.grab() == sl.ERROR_CODE.SUCCESS:
        ...
        if len(boxes) > 0:
            ...
            confidance = np.array(confidance)
            try:
                tracks = tracker.update(output_results = np.concatenate((boxes, confidance), axis=1), img_info = (720, 1280), img_size= (720, 1280))
            except ValueError as e:
                print(e)
                print("-------------------------------")
                print(f"track_ids:{track_ids} b:{b} tracks:{tracks} confidance:{confidance}" )
                print("-------------------------------")

            if tracks:
                b = np.array([bb for bb in boxes])
                try:
                    track_ids = match_detections_with_tracks(tlbr_boxes=b, track_ids=track_ids, tracks=tracks)            
                except:
                    print("ERROR _______________________________________________________________ ERROR")
                    print(f"track_ids:{track_ids} b:{b} tracks:{tracks} confidance:{confidance}" )
                    y_trt.destroy()
